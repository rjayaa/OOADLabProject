...
public class WaiterOrderPage extends Stage {
			...
			protected void updateItem(Void item, boolean empty) {
				...
				if (empty) {
					...
					updateButton.setOnAction(event -> {
						...
						orderController.updateOrderStatusByWaiter(curr.getOrderId(), curr.getOrderItems());
						reloadPage();
					});
					
					deleteButton.setOnAction(event -> {
						Order curr = getTableView().getItems().get(getIndex());
						int orderCost = curr.getOrderTotal();
					...
					else if(curr.getOrderStatus().equals("Prepared")) {
	...
	private void loadMenuItemsData(TableView<Order> menuOrderTable) {
		ArrayList<Order> showOrderList = orderController.showOrderListForWaiter();
		menuOrderTable.getItems().setAll(showOrderList);
	}

	private void reloadPage() {
	    Platform.runLater(() -> {
	    	WaiterOrderPage wop = new WaiterOrderPage();
			wop.show();
			close();
	    });
	}
